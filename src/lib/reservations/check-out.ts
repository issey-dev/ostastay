...
// Check a guest out — the whole desk check-out (balance and City Ledger guards, folio
// closing, debtor-account transfer, commission, activity log). Moved out of
// POST /api/reservations/[id]/check-out so Night Audit's departures step can check out
// settled guests with the very same rules (PropertySettings.autoCheckOutZeroBalance). The
// caller checks the permission; this checks that the reservation is the caller's.
export async function checkOutReservation(ctx: AuthContext, id: string, opts: { early?: boolean } = {}): Promise<NextResponse> {
