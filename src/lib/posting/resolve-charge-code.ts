...
//
//   1. the property's own pointer in PropertySettings (what it picked in the Hub under
//      Charge Codes › Posting Defaults), then
...
//   3. the bare code string, as a last resort for a chart seeded before the hierarchy
//      existed and never backfilled.
//
// Always within ONE property's chart (per property since 2026-09-23). The first argument
// is an object on purpose: an enterprise id and a property id are both strings, and a
// bare-string signature would let an old caller pass the wrong one and silently find
// nothing.
//
...
/**
 * Resolve the charge code that plays `role` at this property, or null when the property
 * genuinely has none (callers decide whether that's fatal — a missing ACCOMMODATION code
 * blocks Night Audit, a missing GREEN_TAX code only matters when Green Tax is on).
 *
 * `settings` may be passed when the caller already loaded the property's settings, to
 * avoid a second read on the hot Night Audit path.
 */
export async function resolveChargeCode(
  ...
  const settings =
    opts?.settings !== undefined
      ? opts.settings
      : await db.propertySettings.findUnique({ where: { propertyId } });
  ...
  if (pointerId) {
    ...
    const byPointer = await db.chargeCode.findFirst({
      where: { id: pointerId, propertyId, isActive: true },
      include: TAX_INCLUDE,
    });
  ...
  const bySystemFlag = await db.chargeCode.findFirst({
    where: { propertyId, code: seededCode, isSystem: true, isActive: true },
    include: TAX_INCLUDE,
  });
  if (bySystemFlag) return bySystemFlag;

  return db.chargeCode.findFirst({
    where: { propertyId, code: seededCode },
    include: TAX_INCLUDE,
  });
}
...
export async function requireChargeCode(
  ...
  const code = await resolveChargeCode(scope, role, opts);
  if (!code) {
