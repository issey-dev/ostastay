// ISO 3166-1 country reference data — alpha-2, alpha-3, and common English short name.
// Used to (a) turn a passport MRZ's alpha-3 code into a readable name for eRegistration
// OCR auto-fill, and (b) resolve a flag icon for any of the various ways a "country"
// value shows up in this app: an enterprise's NATIONALITY SystemCode (alpha-2, see
// scripts/seed/seed-veyo.ts), a guest-typed free-text value from eRegistration, or an
// OCR-derived alpha-3/name. Deliberately a plain data table, not a dependency — the
// country-flag-icons package (added alongside this) only ships flag SVGs, no names.
export type CountryRecord = { alpha2: string; alpha3: string; name: string };
...
const COUNTRIES: CountryRecord[] = [
  ...
  { alpha2: "CD", alpha3: "COD", name: "DR Congo" },
  ...
  { alpha2: "PN", alpha3: "PCN", name: "Pitcairn Islands" },
  ...
  { alpha2: "VI", alpha3: "VIR", name: "United States Virgin Islands" },
...
// Accepts an alpha-2 code, an alpha-3 code, or an already-resolved country name — this
// app has all three depending on the path the value came in through (enterprise
// SystemCode, guest-typed eRegistration text, OCR'd MRZ). Returns undefined rather than
...
export function findCountry(value: string | null | undefined): CountryRecord | undefined {
  ...
  if (v.length === 3) return BY_ALPHA3.get(v) ?? BY_NAME.get(v);
...
// Alpha-2 code for flag lookup, or undefined if the value isn't a recognized country —
// deliberately does NOT fall back to guessing from a demonym (e.g. "British" has no
// reliable single-country mapping), matching the existing "flag only for a real code"
// convention already in use on the reservation detail page.
