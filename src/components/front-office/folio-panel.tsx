...
export function FolioPanel({ reservationId, propertyId, isOpen, onClose }: FolioPanelProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-full sm:max-w-[95vw] w-full h-[100dvh] sm:h-[95vh] p-6 sm:p-8 flex flex-col bg-slate-50 overflow-y-auto">
        ...
        {loading && folios.length === 0 ? (
          <div className="flex-1 flex justify-center items-center text-slate-500">Loading Folio...</div>
        ) : (
          <div className="flex flex-col flex-1 h-full">
            {/* Folio Tabs */}
            <div className="flex items-center gap-2 mb-4 overflow-x-auto pb-2 shrink-0 border-b border-slate-200">
              {folios.map((f: any) => {
              ...
              <Button variant="ghost" size="sm" onClick={handleAddFolio} className="ml-2 h-8 text-slate-500">
                <Plus className="w-4 h-4 mr-1" /> Add Folio
              </Button>
            </div>

            {!activeFolio ? (
              <div className="flex-1 flex justify-center items-center text-slate-500">No folio found for this reservation.</div>
            ) : (
              <div className="grid lg:grid-cols-3 gap-6 flex-1 items-start min-h-0">
                ...
                <div className="lg:col-span-2 flex flex-col gap-6 h-full min-h-0">
                  {/* Balance Card */}
                  <div className="bg-white p-6 rounded-xl border shadow-sm flex justify-between items-center shrink-0">
                    <div>
                      <p className="text-sm font-medium text-slate-500">Folio {activeFolio.folioNumber} Balance</p>
                      ...
                      <div className="mt-1 mb-3 flex items-center gap-2">
                        ...
                        <Button variant="link" size="sm" className="h-auto p-0 text-indigo-600" onClick={() => {
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <p className={`text-4xl font-bold ${balance > 0 ? 'text-rose-600' : balance < 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
                          ${balance.toFixed(2)}
                        </p>
                        <Button 
                          size="sm" 
                          ...
                          className="h-9 shadow-sm border-slate-200 ml-2"
                        >
                    ...
                    <div className="text-right text-sm text-slate-500 space-y-1">
                      <p>Base Charges: <span className="font-medium text-slate-900">${totalBaseCharges.toFixed(2)}</span></p>
                      <p>Service Charge: <span className="font-medium text-slate-900">${totalServiceCharges.toFixed(2)}</span></p>
                      <p>Total Taxes: <span className="font-medium text-slate-900">${totalTaxes.toFixed(2)}</span></p>
                      <p className="border-t pt-1 mt-1">Total Payments: <span className="font-medium text-slate-900">${totalPayments.toFixed(2)}</span></p>
                    </div>
                  </div>

                  {/* Move Action Bar (Shows when items selected) */}
                  {selectedLineItemIds.length > 0 && (
                    <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-3 flex justify-between items-center shrink-0">
                      <span className="text-sm font-medium text-indigo-700">
                        {selectedLineItemIds.length} charge(s) selected
                      </span>
                      <Button size="sm" onClick={() => setIsMoveDialogOpen(true)} className="bg-indigo-600 hover:bg-indigo-700">
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {activeFolio.lineItems.map((item: any) => (
                            <TableRow key={item.id} className={selectedLineItemIds.includes(item.id) ? "bg-indigo-50/30" : ""}>
                              ...
                              <TableCell className="text-xs text-slate-500">{new Date(item.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' }).replace(/ /g, '-')}</TableCell>
                              <TableCell>{item.description}</TableCell>
                              <TableCell className="text-right">${item.amount.toFixed(2)}</TableCell>
                              <TableCell className="text-right text-slate-500">${(item.serviceChargeAmount || 0).toFixed(2)}</TableCell>
                              <TableCell className="text-right text-slate-500">${item.taxAmount.toFixed(2)}</TableCell>
                              <TableCell className="text-right font-medium text-rose-600">${(item.amount + (item.serviceChargeAmount || 0) + item.taxAmount).toFixed(2)}</TableCell>
                          ...
                          {activeFolio.payments.map((payment: any) => (
                            <TableRow key={payment.id}>
                              <TableCell></TableCell>
                              <TableCell className="text-xs text-slate-500">{new Date(payment.createdAt).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: '2-digit' }).replace(/ /g, '-')}</TableCell>
                              <TableCell>Payment - {payment.paymentMethod?.name}</TableCell>
                              <TableCell className="text-right text-slate-300">-</TableCell>
                              <TableCell className="text-right text-slate-300">-</TableCell>
                              <TableCell className="text-right text-slate-300">-</TableCell>
                              <TableCell className="text-right text-slate-300">-</TableCell>
                              <TableCell className="text-right font-medium text-emerald-600">${payment.amount.toFixed(2)}</TableCell>
                            </TableRow>
                          ))}
                          {activeFolio.lineItems.length === 0 && activeFolio.payments.length === 0 && (
                            <TableRow>
                              <TableCell colSpan={8} className="text-center py-12 text-slate-500">No transactions posted yet.</TableCell>
                            </TableRow>
                          )}
                        </TableBody>
                      </Table>
                ...
                <div className="lg:col-span-1 flex flex-col gap-6 sticky top-0 shrink-0">
                  <Tabs value={postType} onValueChange={(v: any) => setPostType(v)} className="w-full">
                    ...
                    <TabsContent value="charge" className="bg-white p-5 rounded-b-xl border border-t-0 shadow-sm mt-0">
                      <form onSubmit={handlePostCharge} className="grid gap-5">
                        <div className="space-y-2">
                          <Label>Charge Code <span className="text-red-500">*</span></Label>
                    ...
                    <TabsContent value="payment" className="bg-white p-5 rounded-b-xl border border-t-0 shadow-sm mt-0">
                      <form onSubmit={handlePostPayment} className="grid gap-5">
                        ...
                        <Button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 mt-2" disabled={loading}>Post Payment to Folio {activeFolio.folioNumber}</Button>
                      </form>
                    </TabsContent>
                  </Tabs>
                </div>
