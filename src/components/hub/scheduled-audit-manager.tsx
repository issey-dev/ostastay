...
const AUTO_AUDIT_HINT =
  "Runs every End-of-Day step with no one at the desk. It stops at the first step that needs a person — guests still due out, for example (settled ones can be checked out automatically, under Departures) — and the Hub Overview says so; finish it from the Night Audit screen."
        ...
        <FormField control={form.control} name="autoAuditEnabled" render={({ field }) => (
          <FormItem className="flex items-start justify-between gap-4 rounded-md border border-border p-3">
            <div className="min-w-0">
              <FormLabel>
                Run Night Audit automatically
                <PhoneHint label="Run Night Audit automatically">{AUTO_AUDIT_HINT}</PhoneHint>
              </FormLabel>
              <FormDescription className="max-sm:hidden">{AUTO_AUDIT_HINT}</FormDescription>
            </div>
            <FormControl>
