...
export default async function HubPropertyChargeCodesPage({ params }: { params: Promise<{ slug: string; propertyId: string }> }) {
  const { ctx, property, item } = await propertyPage(params, "cashiering")
  ...
  return (
    <div className="space-y-6">
      ...
      <ControlsCard title="Charge Codes" description="The transaction codes this property posts against — Night Audit, POS, folios and billing. Each is classified by Subgroup (that's what reports group by), carries a posting type, and can automatically generate derived charges such as Green Tax.">
