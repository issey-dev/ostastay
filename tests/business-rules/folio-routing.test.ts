...
describe("Folio routing + cross-folio move", () => {
  ...
  beforeAll(async () => {
    ...
    const cc = await customChargeCode({ propertyId }, { code: "TRF", description: "Transfer" });
    chargeCodeId = cc.id;
    const passwordHash = await bcrypt.hash("password123", 10);
    const admin = await prisma.user.create({ data: { enterpriseId, email: `rt-admin-${uniq()}@test.local`, passwordHash, firstName: "Admin", lastName: "RT", roles: { create: { roleId: roleIds["Admin"] } }, scope: "ENTERPRISE" } });
    adminId = admin.id;
