...
describe("Business date: helpers, checkout gating", () => {
  ...
  beforeAll(async () => {
    ...
    const admin = await prisma.user.create({ data: { enterpriseId, email: `bd-admin-${uniq()}@test.local`, passwordHash, firstName: "Admin", lastName: "BD", roles: { create: { roleId: roleIds["Admin"] } }, scope: "ENTERPRISE" } });
    adminId = admin.id;
