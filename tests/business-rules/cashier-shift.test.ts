...
describe("Cashier shift: per-user/property, auto-open, charge attribution", () => {
  ...
  beforeAll(async () => {
    ...
    roomCodeId = (await customChargeCode({ propertyId }, { code: `ROOM-${uniq()}`, description: "Room", subgroupCode: "10RV" })).id;
    fbCodeId = (await customChargeCode({ propertyId }, { code: `FB-${uniq()}`, description: "Food", subgroupCode: "20RV" })).id;
