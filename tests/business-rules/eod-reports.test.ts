...
describe("EOD report snapshots", () => {
  ...
  beforeAll(async () => {
    ...
    roomChargeCodeId = (await customChargeCode({ propertyId }, { code: `ROOM-${uniq()}`, description: "Room Revenue", subgroupCode: "10RV" })).id;
    fbChargeCodeId = (await customChargeCode({ propertyId }, { code: `FB-${uniq()}`, description: "Restaurant", subgroupCode: "20RV" })).id;
    cashMethodId = (await prisma.paymentMethod.create({ data: { enterpriseId, propertyId, name: "Cash", type: "CASH" } })).id;
    const cashier = await prisma.user.create({ data: { enterpriseId, email: `er-cash-${uniq()}@test.local`, passwordHash: "x", firstName: "Cash", lastName: "Ier", roles: { create: { roleId: roleIds["Cashier"] } }, scope: "PROPERTY", propertyId } });
