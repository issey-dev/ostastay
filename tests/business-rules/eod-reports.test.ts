...
describe("EOD report snapshots", () => {
  ...
  beforeAll(async () => {
    ...
    roomChargeCodeId = (await customChargeCode(enterpriseId, { code: `ROOM-${uniq()}`, description: "Room Revenue", subgroupCode: "10RV" })).id;
    fbChargeCodeId = (await customChargeCode(enterpriseId, { code: `FB-${uniq()}`, description: "Restaurant", subgroupCode: "20RV" })).id;
